"""Data models for the ``geo-query`` server (Pillar B, expansion).

``geo-query`` runs a spatial SQL query against a configured engine (DuckDB
Spatial or Amazon Athena) over data in place and returns a tabular result set.
This module defines the single result type that ``spatial_sql`` returns:

* :class:`ResultSet` - a column-oriented header plus row-major values, so the
  result of any engine's spatial SQL query is shaped uniformly regardless of
  which engine produced it (Requirement 8.6).

Python 3.9+ compatibility (matching ``geo-common``): uses ``from __future__
import annotations`` together with ``typing`` generics so the pydantic model
resolves its annotations cleanly.
"""
...
class ResultSet(BaseModel):
    ...
    and ``engine`` records which query engine produced the result so callers
    can attribute provenance. An empty query result is a valid ``ResultSet``
    with no rows.
    """

    engine: str = Field(min_length=1, description="The query engine that executed the query.")
