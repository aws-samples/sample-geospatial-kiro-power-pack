...
#: includes at least minimum, maximum, mean, sum, and count; ``std`` is the
#: population standard deviation (ddof=0), matching the rasterstats/NumPy
#: convention this module mirrors (a single-cell zone yields ``0.0``).
SUPPORTED_STATISTICS: Tuple[str, ...] = ("min", "max", "mean", "sum", "count", "std")

#: The default statistics returned when a caller does not name a subset. ``std``
#: is supported but opt-in, so the default result shape is unchanged.
