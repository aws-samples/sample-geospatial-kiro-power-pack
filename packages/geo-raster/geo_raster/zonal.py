...
from geo_raster.geometry import geometry_bbox, iter_polygons, point_in_geometry
...
async def zonal_statistics(
    *,
    ...
    zone's ``min``/``max``/``mean``/``sum``/``count`` over the cells whose center
    falls inside it. Zones with no overlapping cells receive a no-data indication
    while the others still receive statistics (Requirement 8.8). On an S3 read
    failure after the shared client's retry attempts within the 30-second
    window, the operation aborts with local storage unchanged and a ``network``
    Error_Taxonomy error indicating the S3 read failure (Requirement 12.6).
    """
    requested = _validate_stats(stats, source=raster_href if isinstance(raster_href, str) else "geo-raster")
