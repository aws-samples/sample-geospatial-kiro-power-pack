"""Catalog/credential registration + error sweep for ``geo-terrain`` (task 13.6).

Cross-cutting registration half for the Pillar A expansion server
``geo-terrain``:

* the ``elevation`` capability registers as a Resource Catalog entry naming
  ``geo-terrain`` as provider, with the ``uvx`` install command for the
...
* an unreachable / non-responding source surfaces as an ``Error_Taxonomy``
  availability error (``NETWORK``) identifying the logical terrain source, with
  no partial data (Requirement 7.11); and
...
def _manifest_entry() -> Dict[str, Any]:
    ...
    assert "geo-terrain" in servers
    return servers["geo-terrain"]
...
def _server(handler: Any) -> GeoTerrainServer:
    client = HttpClient(
        RetryPolicy(max_attempts=1),
        transport=httpx.MockTransport(handler),
        sleep=_no_sleep,
    )
    return GeoTerrainServer(http=client)
...
def test_required_credentials_match_manifest() -> None:
    specs = GeoTerrainServer().required_credentials()
...
async def test_unreachable_source_identifies_logical_terrain_source() -> None:
    ...
    # The default source is SRTM (the public-endpoint-backed dataset).
    assert err.source == "srtm"
