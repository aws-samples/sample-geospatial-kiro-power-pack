...
- Provider tools: `geo-terrain.dem_zonal`, `geo-ops.transform_crs`,
  `geo-ops.simplify`, `geo-formats.to_geoparquet`
...
- CRS alignment is the most common source of silent errors — reproject
  explicitly.
- High-vertex zones bite here too: shrink a detailed watershed/basin perimeter
  with `geo-ops.simplify(preserve_topology=True)` before `dem_zonal` so the
  inline geometry payload stays small, and reproject the *reduced* geometry
