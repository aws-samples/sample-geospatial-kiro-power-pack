...
- Provider tools: `geo-raster.zonal_statistics`,
  `geo-raster.zonal_band_math`, `geo-ops.transform_crs`, `geo-ops.simplify`,
...
3. **Reduce zone complexity before you call.** A statistic is only as light as
   the geometry you pass. If a zone perimeter has hundreds/thousands of vertices
   (a detailed watershed, admin boundary, or fire perimeter), the inline
   coordinate payload — not the raster window — is what makes the call slow or
   fail. Shrink it first with `geo-ops.simplify(geometry=...,
   tolerance=..., preserve_topology=True)`, which keeps the concave shape
   (unlike `convex_hull`, a lossy superset); fall back to `convex_hull` or a
   bounding box only when a coarse zone is acceptable. Where the tools accept a
   path/href, prefer referencing the geometry by location over inlining it.
4. **Compute zonal statistics.** Call
   `geo-raster.zonal_statistics(raster_href=..., zones=..., stats=[...])` to get
   one `ZoneStat` per zone (Requirement 8.7). **Multi-asset index case:** when
   the index you want to summarize (NDVI/NDWI/SAVI/EVI/NBR/…) lives across
   *separate single-band COGs* — as on Earth Search, where each band is its own
   `.tif` — there is no pre-made index raster to point `zonal_statistics` at.
   Use `geo-raster.zonal_band_math(assets={"B08": ".../B08.tif", "B04":
   ".../B04.tif"}, expression="(B08-B04)/(B08+B04)", zones=..., stats=[...])`,
   which computes the true per-pixel index over the zones' window and reduces it
   per zone in one call. The zones must already be in the assets' CRS.
5. **Handle no-data zones.** For any zone with no overlapping raster cells,
   ...
   empty.
6. **Emit GeoParquet.** Write the per-zone results as GeoParquet via
...
- Vector complexity is a distinct scaling axis from raster size: a small raster
  window with a very high-vertex zone can fail on the inline geometry payload
  alone. Reduce the geometry first (`geo-ops.simplify`) and reproject the
  *reduced* geometry, not the full perimeter (`geometry-complexity` and
  `crs-handling` skills).
