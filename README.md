...
| 15 | `geo-foundation-models` | C | Open | 9 | embeddings, change detection, segmentation. `embed_asset`/`detect_change_from_assets` read a COG window server-side (byte-range) so you pass an href + window instead of inline pixels; `embed_assets` stacks **separate single-band COGs** (one per band, e.g. Earth Search's per-band `.tif`s) into one embedding, and `detect_change_from_assets` takes either single multi-band COGs or per-band lists per date (`assets_a`/`assets_b`) so the whole embed-both-dates-and-compare flow is one call; `change_map` returns a per-tile change **grid** over an AOI (tiles → embed both dates → reduce), optionally reduced **per vector zone**. Optional `latlon`/`acquired` are forwarded to location/time-aware backends. On-tile `embed_tile`/`segment` use deterministic local stand-in backends for Clay/Prithvi-EO-2.0/SatCLIP/SAMGeo, pluggable for **real weights** either **in-process** for small workflows (a local embedding callable — set `GEO_FM_EMBED_LOCAL_CALLABLE`) or via a **remote inference endpoint** (generic HTTPS or AWS SageMaker — set `GEO_FM_EMBED_ENDPOINT_URL`/`GEO_FM_SAGEMAKER_ENDPOINT`); `backend` records provenance, `structure_only` flags a no-pixel tile, and change tools loudly caveat (or refuse) under the uncalibrated stand-in. Bring any model with a small `embed(payload)` callable + `GEO_FM_EXTRA_MODELS="Name:Dim"` (see `examples/custom-embedding-backend/` and `examples/clay-local-backend/`; validate with the `RUN_LIVE_EMBED=1` self-test). `lookup_embeddings` retrieves **real** published Clay v1.5 (1024-d) Sentinel-2 vectors; `available_embedding_periods` lists the months those cover |
